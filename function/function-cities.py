...
def trigger_df_function_cities(request: Request):
    ...
    # Only process if the uploaded file is 'dim_cities.csv'
    if file_name != 'dim_cities.csv':
        print(f"Skipping job, {file_name} is not dim_cities.csv")
    ...
    template_body = {
        "jobName": "dim_cities_load",
        "parameters": {
            "inputFilePattern": f"gs://bkt-landing-superstore/{file_name}",
            "JSONPath": "gs://bkt-df-metadata-superstore/bq-dim-cities.json",
            "outputTable": "data-mining-project-440015:superstore_dataset.dim_cities",
            "bigQueryLoadingTemporaryDirectory": "gs://bkt-df-metadata-superstore",
            "javascriptTextTransformGcsPath": "gs://bkt-df-metadata-superstore/udf-dim-cities.js",
