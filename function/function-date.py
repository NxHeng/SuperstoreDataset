...
def trigger_df_function_date(request: Request):
    ...
    # Only process if the uploaded file is 'dim_date.csv'
    if file_name != 'dim_date.csv':
        print(f"Skipping job, {file_name} is not dim_date.csv")
    ...
    template_body = {
        "jobName": "dim_date_load",
        "parameters": {
            "inputFilePattern": f"gs://bkt-landing-superstore/{file_name}",
            "JSONPath": "gs://bkt-df-metadata-superstore/bq-dim-date.json",
            "outputTable": "data-mining-project-440015:superstore_dataset.dim_date",
            "bigQueryLoadingTemporaryDirectory": "gs://bkt-df-metadata-superstore",
            "javascriptTextTransformGcsPath": "gs://bkt-df-metadata-superstore/udf-dim-date.js",
