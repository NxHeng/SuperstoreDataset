...
def trigger_df_function_products(request: Request):
    ...
    # Only process if the uploaded file is 'dim_products.csv'
    if file_name != 'dim_products.csv':
        print(f"Skipping job, {file_name} is not dim_products.csv")
    ...
    template_body = {
        "jobName": "dim_products_load",
        "parameters": {
            "inputFilePattern": f"gs://bkt-landing-superstore/{file_name}",
            "JSONPath": "gs://bkt-df-metadata-superstore/bq-dim-products.json",
            "outputTable": "data-mining-project-440015:superstore_dataset.dim_products",
            "bigQueryLoadingTemporaryDirectory": "gs://bkt-df-metadata-superstore",
            "javascriptTextTransformGcsPath": "gs://bkt-df-metadata-superstore/udf-dim-products.js",
